...
### Build all examples
From the `ComputerCard_Examples/` directory:
- Run `make all`
- The example cards in `examples/` will be built, and the `.uf2` files placed in the `build/` directory

From the repository root:
- Run `make ComputerCard_Examples`

### Build a single example
From the `ComputerCard_Examples/` directory:
- Run `make <example>` (for example: `make passthrough`)

From the repository root:
- Run `make ComputerCard_Examples/<example>` (for example: `make ComputerCard_Examples/passthrough`)

Flash the resulting `.uf2` file to the Workshop System Computer in the usual way.

By default, built `.uf2` files are also copied into `UF2/` for convenience (so they can be committed if you want).
You can override this location by configuring CMake with `-DCOMPUTERCARD_UF2_DIR=<path>`.

### Speeding up repeated builds (dependency caching)
Some Pico SDK tooling/dependencies may be fetched by CMake (for example `picotool`). By default, the build scripts will cache these in `~/.cache/computercard/fetchcontent` so they only download once per dev container.

You can override the cache location by setting `COMPUTERCARD_DEPS_DIR`, e.g.
- `export COMPUTERCARD_DEPS_DIR=/workspaces/.computercard-deps`

### Cleaning build artifacts
- `make clean` removes `build/`.
- If you previously ran CMake directly in this directory (creating `CMakeFiles/`, `_deps/`, etc), run `make scrub` to remove those legacy in-source build artifacts. (This does not remove `UF2/`.)
